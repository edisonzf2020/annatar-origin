...
from annatar.utils.enums import NudityStatus
from annatar.debrid.models import TorrentStreams
from annatar.utils import const
...
class SortingOption(BaseModel):
    """Sorting option for torrents."""
    key: str = Field(..., description="Key to sort by")
...
class UserData(BaseModel):
    ...
    torrent_sorting_priority: List[SortingOption] = Field(
        default_factory=lambda: [
            SortingOption(key=key) for key in const.TORRENT_SORTING_PRIORITY
        ],
        alias="tsp",
    )
